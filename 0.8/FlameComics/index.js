...
const FlameComicsParser_1 = require("./FlameComicsParser");
...
class FlameComics {
    ...
    async getChapters(mangaId) {
        ...
        return (0, FlameComicsParser_1.parseChapters)($);
    }
    async getChapterDetails(mangaId, chapterId) {
        ...
        this.CloudFlareError(response.status);
        const $ = this.cheerio.load(response.data);
        return (0, FlameComicsParser_1.parseChapterDetails)($, mangaId, chapterId);
    }
    async getMangaDetails(mangaId) {
        ...
        this.CloudFlareError(response.status);
        const $ = this.cheerio.load(response.data);
        return (0, FlameComicsParser_1.parseMangaDetails)($, mangaId);
    }
    async getHomePageSections(sectionCallback) {
        ...
        this.CloudFlareError(response.status);
        const $ = this.cheerio.load(response.data);
        return (0, FlameComicsParser_1.parseHomeSections)($, sectionCallback);
    }
    async getViewMoreItems(homepageSectionId, metadata) {
        ...
        if (homepageSectionId == '2')
            url = `${FC_DOMAIN}/series/?page=${page}&order=update`;
        else if (homepageSectionId == '3')
            url = `${FC_DOMAIN}/series/?page=${page}?status=&type=&order=popular`;
        const request = App.createRequest({
            url,
            method: 'GET'
        });
        ...
        this.CloudFlareError(response.status);
        const $ = this.cheerio.load(response.data);
        const manga = (0, FlameComicsParser_1.parseViewMore)($);
        page++;
        if (manga.length < 10)
            page = -1;
    ...
    async getSearchResults(query, metadata) {
        ...
        this.CloudFlareError(response.status);
        const $ = this.cheerio.load(response.data);
        const manga = (0, FlameComicsParser_1.parseSearchResults)($);
        page++;
        if (manga.length < 10)
            page = -1;
        return App.createPagedResults({
...
},{"./FlameComicsParser":71,"@paperback/types":61}],71:[function(require,module,exports){
"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.parseHomeSections = exports.parseViewMore = exports.parseSearchResults = exports.parseChapterDetails = exports.parseChapters = exports.parseMangaDetails = void 0;
const types_1 = require("@paperback/types");
...
const parseMangaDetails = ($, mangaId) => {
    ...
    const tags = [App.createTagSection({ id: '0', label: 'genres', tags: arrayTags.map((x) => App.createTag(x)) })];
...
const parseHomeSections = ($, sectionCallback) => {
    const section1 = App.createHomeSection({ id: '1', title: 'Popular', type: types_1.HomeSectionType.featured, containsMoreItems: false });
    const section2 = App.createHomeSection({ id: '2', title: 'Latest', type: types_1.HomeSectionType.singleRowNormal, containsMoreItems: true });
    const section3 = App.createHomeSection({ id: '3', title: 'Popular Titles', type: types_1.HomeSectionType.singleRowNormal, containsMoreItems: true });
    const featured = [];
    const popular = [];
    const latest = [];
